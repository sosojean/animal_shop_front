...
const AgeResult = (props) => {
    ...
    // 나이 환산
    const getAge = (birthDate, size, species) => {
        // 생년월일을 Date 객체로 변환
    ...
    return (
        <div>
            ...
            {calcData?.species === "강아지" ?
                <p>
                    {getAge(calcData?.birth, calcData?.size, "강아지") === "none" ? 
                        "생년월일 혹은 크기를 정확히 작성해주세요" : 
                        "우리집 강아지의 나이는 " + getAge(calcData?.birth, calcData?.size, "강아지") + 
                        "살이에요!"
                    }
                </p> : calcData?.species === "고양이" ?
                        <p>
                            {getAge(calcData?.birth, calcData?.size, "고양이") === "none" ? 
                                    "생년월일 혹은 크기를 정확히 작성해주세요" : 
                                    "우리집 고양이의 나이는 " + getAge(calcData?.birth, calcData?.size, "고양이") + 
                                    "살이에요!"
                            }
                        </p> :
